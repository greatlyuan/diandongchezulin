...
<body style="width: 100%; height: 100%; margin: 0; padding: 0; background: url(images/5.jpg) no-repeat;">
        ...
        <div class="row">
            <div class="col-md-offset-3 col-md-6">
                ...
                <form class="form-horizontal" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" complete="on">
                    ...
                    <div class="form-group has-feedback">
                        <div class="input-group">
                            <input type="password" name="check-pwd" id="check-pwd" class="form-control" placeholder="确认密码" value="<?php echo isset($_POST['check-pwd']) ? $_POST['check-pwd'] : ''; ?>">
                            <i class="fa fa-lock"></i>
                        </div>
                        <span style="color:red;display: none;margin-left: 8px" class="tips"></span>
                        <span style="display: none;right: 5px!important;top: 2px;" class="glyphicon glyphicon-remove form-control-feedback"></span>
                    ...
                    <div class="form-group">
                        <a class="text" style="cursor: pointer" href="login.php">有账号？去登录</a>
    ...
    <script>
        ...
        $('.container').find('input').eq(2).change(function() {
            if ($('#check-pwd').val().length == 0) {
                fail($('#check-pwd'), 2, '确认密码不能为空');
            }else{
                if ($('#check-pwd').val() == $('#inputPassword').val()) {
                    success($('#check-pwd'), 2);
                } else {
                    fail($('#check-pwd'), 2, '两次输入的密码不一致');
                }
            }
        });
